const products = [
  {
    id: 201,
    name: "Laptop",
    category: "Electronics",
    price: 999.99,
  },
  {
    id: 202,
    name: "T-shirt",
    category: "Clothing",
    price: 19.99,
  },
  {
    id: 203,
    name: "Coffee Maker",
    category: "Kitchen Appliances",
    price: 79.99,
  },
  {
    id: 204,
    name: "Running Shoes",
    category: "Footwear",
    price: 59.99,
  },
  {
    id: 205,
    name: "Bookshelf",
    category: "Furniture",
    price: 149.99,
  },
];

/**********
      Question 1:
      You have a function getProductName(product) that:
      - receives a product object
      - returns the name of the product
      Don't forget to uncomment the console.log
      ===
      ANSWER: Laptop
      **********/

function getProductName(product) {
  // Your code here
  return product.name;
}
console.log(getProductName(products[0]));
// console.log(getProductName(products[0]));

/**********
      Question 2:
      You have a function that:
      - receives a product object
      - receives a category name
      - returns true if the product's category matches the provided category name, otherwise returns false
      ===
      ANSWER: true
      **********/

function isProductCategoryMatching(product, category) {
  // Your code here
  if (product.category == category) {
    return true;
  } else {
    return false;
  }
}
console.log(isProductCategoryMatching(products[1], "Clothing"));

// console.log(isProductCategoryMatching(products[1], "Clothing"));

/**********
      Question 3:
      addProduct(products, product):
      - receives an array of product objects
      - receives a new product object (with id, name, category, and price)
      - adds the new product to the array
      - returns the updated array
      ===
      ANSWER: 
      [
        {
          "id": 201,
          "name": "Laptop",
          "category": "Electronics",
          "price": 999.99
        },
        {
          "id": 202,
          "name": "T-shirt",
          "category": "Clothing",
          "price": 19.99
        },
        {
          "id": 203,
          "name": "Coffee Maker",
          "category": "Kitchen Appliances",
          "price": 79.99
        },
        {
          "id": 204,
          "name": "Running Shoes",
          "category": "Footwear",
          "price": 59.99
        },
        {
          "id": 205,
          "name": "Bookshelf",
          "category": "Furniture",
          "price": 149.99
        },
        {
          "id": 206,
          "name": "Headphones",
          "category": "Electronics",
          "price": 149.99
        }
      ]
      **********/

function addProduct(products, product) {
  // Your code here
  products.push(product);
  return products;
}

const newProduct = {
  id: 206,
  name: "Headphones",
  category: "Electronics",
  price: 149.99,
};

// const newProduct = {
//   "id": 206,
//   "name": "Headphones",
//   "category": "Electronics",
//   "price": 149.99
// };
console.log(addProduct(products, newProduct));
// console.log(addProduct(products, newProduct));

/**********
      Question 4:
      countElectronicsProducts(products):
      - receives an array of product objects
      - returns the number of products in the "Electronics" category
      ===
      ANSWER: 2
      **********/

function countElectronicsProducts(products) {
  // Your code here
  return products.filter((product) => product.category == "Electronics").length;
}
console.log(countElectronicsProducts(products));
// console.log(countElectronicsProducts(products));

/**********
      Question 5: 🌶️
      listProductNamesByCategory(products, category):
      - receives an array of product objects
      - receives a category name
      - returns an array of product names that belong to the specified category
      ===
      ANSWER: ["Laptop", "Headphones"]
      **********/

// function printProductName(productsList) {
//   console.log(productsList.forEach());
// }

function listProductNamesByCategory(products, category) {
  // Your code here

  const result1 = products.filter((product) => product.category == category);
  const result2 = result1.map((x) => x.name);
  return result2;
}
console.log(listProductNamesByCategory(products, "Electronics"));

// return products.forEach(
//     products.filter((product) => product.category == category)
//   ).name;
// const result = [];
// result = products.map(
//   products.filter((product) => product.category == category)
// );
// return result;
// //
// function printNumName(numName1, numName2, numName3) {
//     console.log(numName1);
//     console.log(numName2);
//     console.log(numName3);
//   }

//   numeros.forEach(printNumName);
// //
// const resultArray1 = [];
// const resultArray2 = [];

// resultArray1 = products.filter((product) => product.category == category);
// for (let i = 0; i < resultArray1.length; i++) {
//   resultArray2[i] = resultArray1[i].name;
//   return resultArray2;
// }
// return resultArray2;
// console.log(listProductNamesByCategory(products, "Electronics"));
// return array.forEach(element => {
//
//});

//
//  const result2 = result1.forEach(result1.name);
//return result2;
//return products.filter((product) => product.category == category).name;

// const productNamesThatBelong = products.filter(
//     (product) => product.category == category
//   );
//   const count = 0;
//   if (productNamesThatBelong.length > count) {
//     const result = [];
//     result.push(productNamesThatBelong[count].name);
//     const count = count + 1;
//   } else {
//     return result;
//   }
//
// console.log(listProductNamesByCategory(products, "Electronics"));

/**********
    Question 6: 🌶️🌶️
    You have a function getCheapestProduct(products) that:
    - receives an array of product objects
    - returns the product object with the lowest price
    ===
    ANSWER: 
    {
      "id": 202,
      "name": "T-shirt",
      "category": "Clothing",
      "price": 19.99
    }
    **********/

function getCheapestProduct(products) {
  // Your code here
  let lowest = products[0];
  products.forEach((product) => {
    if (lowest.price >= product.price) {
      lowest = product;
    }
  });
  return lowest;
}

console.log(getCheapestProduct(products));
//
//  // Your code here
//  const productsPrices = products
//  .filter((product) => product.price >= 0)
//  .map((product) => {
//    return product.price;
//  });
// const lowestPrice = productsPrices;
// return lowestPrice;
//
//
//   // Your code here
//   const productsPrices = products
//     .filter((product) => product.price >= 0)
//     .map((product) => {
//       return product.price;
//     });
//   const lowestPrice = productsPrices.filter(
//     (price) => price <= productsPrices.price
//   );
//   return lowestPrice;
// console.log(getCheapestProduct(products));
//
//   const smallestPrice = 0;
//   for (let i = 0; i <= productsPrices.length; i++) {
//     if (productsPrices[i] < productsPrices[i + 1]) {
//       smallestPrice = productsPrices[i];
//       return smallestPrice;
//     } else {
//       smallestPrice = productsPrices[i + 1];
//       return smallestPrice;
//     }
//   }
//   return products.find((product) => product.price == smallestPrice);
//
// //
// products.forEach((product) => {
//     return productsPrices.push(product.price);
//   });
//
// let smallest = 0;
// for (let i = 0; i <= products.length; i++) {
//   if (products[i].price < products[i + 1].price) {
//     smallest = products[i].price;
//   } else {
//     smallest = products[i + 1].price;
//   }
// }
// const result = [];
// result = products.map((product) => product.price == smallest);
// return result;

//
// const result1 = products.filter(
//     (product) => product.price < product.price.min
//   );
//   const result2 = result1.map((product) => product.price == result1.price);
//   return result2;

// const smallest = 0;
// for (let i = 0; i < products.length; i++) {
//   if (products[i] < products[i + 1]) {
//     smallest = products[i];
//   } else {
//     const result = [];
//     result = products.map((x) => x.price == smallest);
//     return result;
//   }
// }

// const result1 = [];
// const result2 = products.filter((product) =>
//   result1.push(product.price.Math.min)
// );
// return result2;
/**********
      Question 7:
      You have a function removeProductById(products, productId) that:
      - receives an array of product objects
      - receives a product id (number)
      - removes the product with the matching id from the array
      - returns the updated array
      ===
      ANSWER: 
      [
        {
          "id": 201,
          "name": "Laptop",
          "category": "Electronics",
          "price": 999.99
        },
        {
          "id": 203,
          "name": "Coffee Maker",
          "category": "Kitchen Appliances",
          "price": 79.99
        },
        {
          "id": 204,
          "name": "Running Shoes",
          "category": "Footwear",
          "price": 59.99
        },
        {
          "id": 205,
          "name": "Bookshelf",
          "category": "Furniture",
          "price": 149.99
        }
      ]
      //note: 
      //note:
      //note:
      //note: the answer does not take into account adding
      //note: a new product in the previous Question 3 part
      //note: after calling the addProduct function.
      //note:
      //note:
      //note:
      **********/

function removeProductById(products, productId) {
  // Your code here
  return products.filter((product) => product.id != productId);
}
console.log(removeProductById(products, 202));
// console.log(removeProductById(products, 202));

/**********
      Question 8: 🌶️🌶️🌶️
      getProductsByPriceRange(products, minPrice, maxPrice):
      - receives an array of product objects
      - receives a minimum price (number) and a maximum price (number)
      - returns an array of product objects that fall within the specified price range (inclusive)
      ===
      ANSWER: 
      [
        {
          "id": 202,
          "name": "T-shirt",
          "category": "Clothing",
          "price": 19.99
        },
        {
          "id": 204,
          "name": "Running Shoes",
          "category": "Footwear",
          "price": 59.99
        }
      ]
      **********/

function getProductsByPriceRange(products, minPrice, maxPrice) {
  // Your code here
  return products.filter(
    (product) => product.price >= minPrice && product.price <= maxPrice
  );
}
console.log(getProductsByPriceRange(products, 19.99, 59.99));
// console.log(getProductsByPriceRange(products, 19.99, 59.99));
// thanks for the opportunity
// you guys are the best
